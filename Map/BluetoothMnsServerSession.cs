...
namespace GoodTimeStudio.MyPhone.OBEX.Map
{
    public class BluetoothMnsServerSession : BluetoothObexServerSession<MnsServer>
    {
        ...
        public BluetoothMnsServerSession(CancellationTokenSource token) : base(MAP_MNS_Id, 1, token)
        {
        }

        protected override MnsServer CreateObexServer(StreamSocket clientSocket, CancellationTokenSource token)
        {
