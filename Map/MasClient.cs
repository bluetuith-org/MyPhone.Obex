...
public class MasClient : ObexClient
{
    ...
    )
    {
        ...
        var resp = await RunObexRequestAsync(packet);
        var listingObj = resp.GetBodyContentAsUtf8String(false);
    ...
    public async Task<ushort> GetMessageListingSizeAsync(string folderName = "telecom")
    {
        ...
        var response = await RunObexRequestAsync(packet);
        var paramDict = response
    ...
    public async Task<BMessage> GetMessageAsync(string messageHandle)
    {
        var packet = new ObexPacket(
            ...
            new AppParameterHeaderBuilder(
                new AppParameter((byte)MasAppParamTagId.Attachment, MasConstants.AttachmentOn),
                new AppParameter((byte)MasAppParamTagId.Charset, MasConstants.CharsetUtf8)
            ).Build()
        );

        var resp = await RunObexRequestAsync(packet);

        var bMsgStr = resp.GetBodyContentAsUtf8String(true);
    ...
    {
        var flag = (byte)(enableNotification ? 1 : 0);
    ...
    )
    {
        ...
        if (maxListCount != null || listStartOffset != null)
        {
            ...
            if (listStartOffset != null)
                builder.AppParameters.Add(
                    new AppParameter((byte)MasAppParamTagId.ListStartOffset, listStartOffset.Value)
                );
        ...
        var objStr = resp.GetBodyContentAsUtf8String(true);
        xml.LoadXml(objStr);
        var list = xml.SelectNodes("/folder-listing/folder/@name");
    ...
    public async Task<List<string>> GetAllChildrenFoldersAsync()
    {
        ...
        while (!lastPage)
        {
            ...
            if (foldersName.Count < defaultSize)
                lastPage = true;
    ...
    public async Task<List<MessageListing>> GetAllMessagesAsync(string folderName = "telecom")
    {
        ...
        while (!lastPage)
        {
            var handles = await GetMessagesListingAsync(offset, defaultSize, folderName);
            if (handles.Count < defaultSize)
                lastPage = true;
    ...
    public async Task<SmsFolder> TraverseFolderAsync(bool getMessageHandles = false)
    {
        ...
        var root = new SmsFolder("Root");
        var folders = new Stack<SmsFolder>();
        folders.Push(root);
        var pre = root;
        ...
        while (folders.Count > 0)
        {
            ...
            var subFoldersName = await GetAllChildrenFoldersAsync();
            foreach (var subFolder in subFoldersName)
            {
                var count = await GetMessageListingSizeAsync(subFolder);
                var smsFolder = new SmsFolder(subFolder, count, current);
                if (getMessageHandles)
                    foreach (var handle in await GetAllMessagesAsync(subFolder))
