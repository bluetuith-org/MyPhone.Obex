...
public class MapSetPathRequestPacket : ObexPacket
{
    ...
    ///     If the <paramref name="mode" /> is <see cref="SetPathMode.BackToParent" />, the folderName is optional (either
    ///     empty or peer folder name).
    ///     If the folderName is empty, go back to the parent folder.
    ///     If the folderName is a peer folder name (child of the parent folder), go back to the parent folder first then enter
    ///     the child folder.
